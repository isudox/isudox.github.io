...
- [x] 什么时候需要commit，我觉得不是写两行代码或者写100行代码就得提交一下，而是完成一个小功能或修复一个bug，并且整个功能能正常运行，不会影响其他开发人员调试，那么就可以提交上去，无论是改了一行代码还是100行代码。另外，commit时认真写注释还是很重要的，既方便自己也方便他人，不至于想回滚时都不知道哪次提交都改了什么。
- [x] 在`push`操作之前，先`pull`远程的代码，因为别人可能已经把他们开发的代码推送上去了，本地版本比远程版本旧，是无法推送上去的。此外，`pull`后可以尽早发现可能的冲突，尽早解决，不然后期一堆冲突文件会抓狂的。
- [x] 暂时没想到更多的注意点，想到了再补上……荆轲刺秦王
